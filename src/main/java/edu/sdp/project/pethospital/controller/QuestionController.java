...
public class QuestionController {
    ...
    @RequestMapping(value = { "/admin/test/question/{quesId}" }, method = RequestMethod.PUT)
    ...
    @RequestMapping(value = { "/admin/test/question/tag/{tag}" }, method = RequestMethod.PUT)
    ...
    /**
     * 本接口支持通过id搜索，格式为：qid:{id1},{id2},... 如果不按以上格式则按照关键词处理，处理方式为返回描述中有匹配关键字的问题
     */
    @ResponseBody
    ...
    /**
     * 新增问题时不用给出id，id自增管理 但必须给出描述（题面），理论上不同问题的描述不应该相同，所以如果插入的问题描述已经存在，则无法插入
     * 其他属性(除image外，image应该在新增问题之后通过上传添加)都是可选项。 但是建议在新建问题时便给出除image外的所有属性
     */
    @ResponseBody
    ...
    ResponseMsg addQuestion(@RequestBody Map param) {
        ResponseMsg msg = new ResponseMsg();
        msg.setStatus(400);
        if (!(param.containsKey("descrip") && param.containsKey("type")))
            return msg;
        msg.setStatus(500);
    ...
    ResponseMsg uploadImage(@PathVariable("quesId") int quesId, @RequestParam("image") MultipartFile image) {
        ResponseMsg msg = new ResponseMsg();
        msg.setStatus(404);
        if (!questionService.checkId(quesId))
            return msg;
        msg.setStatus(500);
    ...
    /**
     * 此方法不建议调用，因为仅删除了数据库中的记录，并没有删除对应的图片 会导致服务器上出现冗余文件 TODO: 增加自动删除相关图片的功能
     */
    @ResponseBody
    ...
    ResponseMsg deleteQuestion(@RequestParam("quesId") int quesId) {
        ...
        if (!questionService.checkId(quesId))
            return msg;
        if (questionService.deleteQuestionById(quesId) > 0)
            msg.setStatus(200);
        return msg;
    }
}
